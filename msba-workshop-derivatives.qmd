...
```{python}
#| label: fig-maximizing-function
#| echo: false 
#| fig-cap: f(x) and its log are maximized at the same x
...
def f(x):
    return x**8 * np.exp(-5 * x)

x = np.linspace(0.5, 4, 500)
x_max = 8 / 5
y_max = f(x_max)

df1 = pd.DataFrame({'x': x, 'y': f(x)})
df2 = pd.DataFrame({'x': x, 'y': np.log(f(x))})

p1 = (ggplot(df1, aes('x', 'y'))
      + geom_line(color='tab:blue', size=1)
      + geom_segment(aes(x=x_max, xend=x_max, y=-np.inf, yend=y_max), linetype='dotted', color='black')
      + labs(y='f(x)'))

p2 = (ggplot(df2, aes('x', 'y'))
      + geom_line(color='tab:red', size=1)
      + geom_segment(aes(x=x_max, xend=x_max, y=-np.inf, yend=np.log(y_max)), linetype='dotted', color='black')
      + labs(y='ln(f(x))'))
...
There are many more [derivative rules](https://en.wikipedia.org/wiki/Differentiation_rules).
